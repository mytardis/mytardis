...
class StorageBox(models.Model):
    ...
    online_types = [CACHE, DISK, TEMPORARY, BUNDLE]
    ...
    type_order = [CACHE, BUNDLE, DISK, TAPE, TEMPORARY, TYPE_UNKNOWN]
    ...
    def storage_type(self):
        try:
            ...
            return StorageBox.TYPES.get(storage_type, StorageBox.TYPE_UNKNOWN)
    ...
    def get_initialised_storage_instance(self):
        storage_class = django_storage.get_storage_class(self.django_storage_class)
    ...
    def cache_box(self):
        ...
        caches = [
            box
            for box in self.child_boxes.all()
    ...
    def get_default_storage(cls, location=None, user=None):
        ...
        if location is None:
            try:
                ...
                return StorageBox.objects.get(
                    attributes__key="default", attributes__value="True"
                )
        ...
        if location is not None or default_location is not None:
            ...
            try:
                return StorageBox.objects.get(
                    options__key="location", options__value=box_location
                )
            except StorageBox.DoesNotExist:
    ...
    def create_local_box(cls, location=None):
        ...
        s_box.save()
        sbo = StorageBoxOption(storage_box=s_box, key="location", value=location)
        sbo.save()
        return s_box
...
class StorageBoxOption(models.Model):
    ...
    storage_box = models.ForeignKey(
        StorageBox, related_name="options", on_delete=models.CASCADE
    )
    key = models.TextField()
    value = models.TextField()
    value_type = models.CharField(max_length=6, choices=TYPE_CHOICES, default=STRING)
    ...
    def __str__(self):
        return "-> ".join(
            [
                ...
                ": ".join(
                    [self.key or "no key", str(self.unpickled_value) or "no value"]
                ),
            ]
        )
...
class StorageBoxAttribute(models.Model):
    ...
    storage_box = models.ForeignKey(
        StorageBox, related_name="attributes", on_delete=models.CASCADE
    )
    key = models.TextField()
    value = models.TextField()
