...
def _add_protocols_and_organizations(request, collection_object, c):
    ...
    if getattr(settings, "USER_AGENT_SENSING", False) and request.user_agent:
        logger.debug("user_agent.os.family: %s" % request.user_agent.os.family)
        cannot_do_zip = request.user_agent.os.family in ["Macintosh", "Mac OS X"]
    else:
        cannot_do_zip = False
    ...
    c["default_format"] = list(
        filter(lambda x: not (cannot_do_zip and x == "zip"), formats)
    )[0]

    from ..download import get_download_organizations

    c["organization"] = get_download_organizations()
    c["default_organization"] = getattr(settings, "DEFAULT_PATH_MAPPER", "classic")


def get_dataset_info(
    dataset, request, include_thumbnail=False, exclude=None
):  # too complex # noqa
    ...
    if exclude is None or "datafiles" not in exclude or "file_count" not in exclude:
    ...
    if dataset.instrument and (exclude is None or "instrument" not in exclude):
        obj["instrument"] = dataset.instrument.name
        obj["show_instr_facil"] = True
        if dataset.instrument.facility and (
            exclude is None or "facility" not in exclude
        ):
    ...
    obj["show_dataset_thumbnails"] = getattr(settings, "SHOW_DATASET_THUMBNAILS", True)
