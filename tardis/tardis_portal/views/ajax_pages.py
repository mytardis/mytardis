...
def experiment_description(request, experiment_id):
    ...
    if settings.ONLY_EXPERIMENT_ACLS:
        c['datafiles'] = DataFile.objects.select_related("dataset").prefetch_related(
                        "dataset__experiments").filter(dataset__experiments__id=experiment_id)
    else:
        c['datafiles'] = DataFile.safe.all(request.user).filter(dataset__experiments__id=experiment_id)
...
def experiment_latest_dataset(request, experiment_id):
    if settings.ONLY_EXPERIMENT_ACLS:
        context = dict(datasets=Dataset.objects.prefetch_related(
                       "experiments").filter(experiments__id=experiment_id))
    else:
        context = dict(datasets=Dataset.safe.all(request.user).filter(
