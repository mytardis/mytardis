...
from ...models.access_control import ExperimentACL, DatasetACL, DatafileACL #ProjectACL,
from ...models import (Experiment, Dataset, DataFile, Schema, #Project,
                       ParameterName, DatafileParameterSet, DatafileParameter,
                       DatasetParameterSet, DatasetParameter,
                       ExperimentParameterSet, ExperimentParameter)#, ProjectParameterSet, ProjectParameter)
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        test_facility = Facility(name="STRESSTEST_Facility",
                                     manager_group=test_manage_group)#,institution=test_institution)
        test_facility.save()
        ...
        for idx_proj in range(n_proj):
            ...
            # Generate random samples for level below'''
        rng_exp_dict = {"1" : [0],#rng.choice(100, size=1, replace=False),
                            "25" : list(range(0, 25)),#rng.choice(100, size=25, replace=False),
                            "50" : list(range(0, 50)),#rng.choice(100, size=50, replace=False),
