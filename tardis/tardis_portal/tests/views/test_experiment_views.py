...
class ExperimentTestCase(TestCase):
    ...
    def test_create_and_edit(self, mock_webpack_get_bundle):
        ...
        experiment_id = resolve(urlparse(created_url).path).kwargs["experiment_id"]
        ...
        acl = ExperimentACL.objects.get(experiment=experiment, user=self.user)
    ...
    def test_dataset_json(self):
        ...
        for item in items:
            ...
            response = client.delete(
                json_url + str(item["id"]), content_type="application/json"
            )
            self.assertEqual(response.status_code, 403)
            # Add the dataset to another experiment with PUT
            new_url = reverse(
                "tardis.tardis_portal.views.dataset_json",
                kwargs={
                    "experiment_id": str(experiments[1].id),
                    "dataset_id": item["id"],
                },
            )
            response = client.put(
                new_url, data=json.dumps(item), content_type="application/json"
            )
            item = json.loads(response.content.decode())
            check_item(item)
            # This dataset should now have two experiments
            self.assertEqual(
                sorted(item["experiments"]), sorted([e.id for e in experiments[:2]])
            )
            ...
            check_item(item)
            self.assertEqual(
                sorted(item["experiments"]), sorted([e.id for e in experiments])
            )
            ...
            self.assertEqual(
                sorted(item["experiments"]), sorted([e.id for e in experiments][1:])
            )
