...
class ParameterTestCase(ModelTestCase):
    def test_parameter(self):
        ...
        df_schema = Schema(
            namespace="http://www.cern.ch/felzmann/schema1.xml", type=Schema.DATAFILE
        )
        df_schema.save()

        ds_schema = Schema(
            namespace="http://www.cern.ch/felzmann/schema2.xml", type=Schema.DATASET
        )
        ds_schema.save()

        exp_schema = Schema(
            namespace="http://www.cern.ch/felzmann/schema3.xml", type=Schema.EXPERIMENT
        )
        exp_schema.save()
        ...
        with self.settings(METADATA_STORE_PATH=os.path.dirname(__file__)):
            ...
            df_parameter = DatafileParameter(
                name=df_parname, parameterset=df_parset, string_value=filename
            )
            df_parameter.save()

            ds_parameter = DatasetParameter(
                name=ds_parname, parameterset=ds_parset, string_value=filename
            )
            ds_parameter.save()

            exp_parameter = ExperimentParameter(
                name=exp_parname, parameterset=exp_parset, string_value=filename
            )
            exp_parameter.save()

            self.assertEqual(
                "<a href='/display/DatafileImage/load/%i/' target='_blank'><img style='width: 300px;' src='/display/DatafileImage/load/%i/' /></a>"
                % (df_parameter.id, df_parameter.id),  # noqa
                df_parameter.get(),
            )

            self.assertEqual(
                "<a href='/display/DatasetImage/load/%i/' target='_blank'><img style='width: 300px;' src='/display/DatasetImage/load/%i/' /></a>"
                % (ds_parameter.id, ds_parameter.id),  # noqa
                ds_parameter.get(),
            )

            self.assertEqual(
                "<a href='/display/ExperimentImage/load/%i/' target='_blank'><img style='width: 300px;' src='/display/ExperimentImage/load/%i/' /></a>"
                % (exp_parameter.id, exp_parameter.id),  # noqa
                exp_parameter.get(),
            )
