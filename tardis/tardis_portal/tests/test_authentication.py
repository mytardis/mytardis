...
class AuthenticationTestCase(TestCase):
    ...
    def testSimpleAuthenticate(self):
        response = self.client.post(
            self.loginUrl,
            {"username": "test", "password": "test", "authMethod": "localdb"},
        )
        ...
        if test_login_failures:
            response = self.client.post(
                self.loginUrl,
                {"username": "test", "password": "test1", "authMethod": "localdb"},
            )
            self.assertEqual(
                response.context["status"], "Sorry, username and password don't match."
            )
            self.client.logout()

            response = self.client.post(
                self.loginUrl,
                {"username": "test1", "password": "test", "authMethod": "localdb"},
            )
            self.assertEqual(
                response.context["status"], "Sorry, username and password don't match."
            )
    ...
    def testManageAuthMethods(self, mock_webpack_get_bundle):
        ...
        self.assertRedirects(response, "/login/?next=" + self.manageAuthMethodsUrl)
    ...
    def test_djangoauth(self):
        ...
        request = factory.post(
            "/login", {"username": "test", "password": "test", "authMethod": "localdb"}
        )
