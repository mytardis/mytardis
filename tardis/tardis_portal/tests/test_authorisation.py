...
class ACLTestCase(TestCase):
    ...
    def testChangeUserPermissions(self, mock_webpack_get_bundle):
        login1 = self.client1.login(username=self.user1.username, password="secret")
        self.assertTrue(login1)

        login3 = self.client3.login(username=self.user3.username, password="secret")
        self.assertTrue(login3)

        # user1 should be see experiment1
        response = self.client1.get("/experiment/view/%i/" % (self.experiment1.id))
        self.assertEqual(response.status_code, 200)

        # user3 should not be able to see experiment1
        response = self.client3.get("/experiment/view/%i/" % (self.experiment1.id))
    ...
    def testReadAccess(self, mock_webpack_get_bundle):
        ...
        response = self.client1.get("/experiment/view/%i/" % (self.experiment2.id))
        self.assertEqual(response.status_code, 403)

        # user1 should be allowed to see experiment3 as it's public
        response = self.client1.get("/experiment/view/%i/" % (self.experiment3.id))
        self.assertEqual(response.status_code, 200)
        ...
        response = self.client1.get(
            "/experiment/control_panel/%i/access_list"
            "/add/user/%s/?authMethod=%s&canRead=true"
            % (self.experiment1.id, non_existent, localdb_auth_key)
        )

        self.assertContains(
            response, "User %s does not exist" % non_existent, status_code=400
        )
        ...
        response = self.client2.get("/experiment/view/%i/" % (self.experiment1.id))
        self.assertEqual(response.status_code, 200)

        # user2 should be also able to see experiment2
        response = self.client2.get("/experiment/view/%i/" % (self.experiment2.id))
        self.assertEqual(response.status_code, 200)

        # user2 should be allowed to see experiment3 as it's public
        response = self.client2.get("/experiment/view/%i/" % (self.experiment3.id))
        self.assertEqual(response.status_code, 200)
        ...
        response = self.client3.get("/experiment/view/%i/" % (self.experiment2.id))
        self.assertEqual(response.status_code, 403)

        # user3 should be able to see experiment3
        response = self.client3.get("/experiment/view/%i/" % (self.experiment3.id))
        self.assertEqual(response.status_code, 200)
        ...
        response = self.client1.get(
            "/group/%i/remove/%s/" % (group.id, self.user3.username)
        )
        self.assertEqual(response.status_code, 200)
        ...
        response = self.client1.get(
            "/experiment/control_panel/%i/access_list"
            "/remove/user/%s/" % (self.experiment1.id, self.user3.username)
        )
        self.assertEqual(response.content, b"OK")
        self.assertEqual(response.status_code, 200)
        ...
        response = self.client1.get(
            "/experiment/control_panel/%i/access_list"
            "/remove/user/%s/" % (9999, self.user3.username)
        )
        self.assertEqual(response.status_code, 403)
        ...
        response = self.client1.get(
            "/experiment/control_panel/%i/access_list"
            "/remove/user/%s/" % (self.experiment1.id, self.user1.username)
        )
        self.assertEqual(
            response.content,
            b"All experiments must have at least "
    ...
    def testWriteAccess(self, mock_webpack_get_bundle):
        ...
        response = self.client1.get(
            "/experiment/control_panel/%i"
            "/access_list/remove/group/%s/" % (self.experiment1.id, group.id)
        )
        self.assertEqual(response.status_code, 200)
        ...
        response = self.client1.get(
            "/experiment/control_panel/%i"
            "/access_list/remove/group/0/" % self.experiment1.id
        )
    ...
    def testOwnedExperiments(self):
        ...
        request = MockRequest()
        request.user = user
        num_exps = Experiment.safe.owned(user=request.user).count()
