...
class RestfulExperimentParameterSet(object):
    ...
    def __str__(self):
        return f"{self.__class__} for {self.form_cls} into {self.schema.namespace}"
    ...
    def _list(self, request, experiment_id):
        ...
        sets = ExperimentParameterSet.objects.filter(
            schema=self.schema, experiment__pk=experiment_id
        )
        return HttpResponse(
            json.dumps([self._get_dict_from_ps(ps) for ps in sets]),
    ...
    def _get(self, request, experiment_id, ps_id):
        ...
        try:
            ps = ExperimentParameterSet.objects.get(
                schema=self.schema, experiment__pk=experiment_id, id=ps_id
            )
            return HttpResponse(
                json.dumps(self._get_dict_from_ps(ps)),
                content_type="application/json; charset=utf-8",
            )
        except Exception:
    ...
    def _create(self, request, experiment_id):
        ...
        ps = ExperimentParameterSet(experiment_id=experiment_id, schema=self.schema)
        ps.save()
    ...
    def _delete(self, request, experiment_id, ps_id):
        ...
        ps.delete()
        return HttpResponse(
            json.dumps(obj), content_type="application/json; charset=utf-8"
        )
