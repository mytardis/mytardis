...
def email_user(subject, template_filename, context, user):
    ...
    msg = EmailMultiAlternatives(
        subject, build_template_text(template_filename, context),
        from_email, to_email)

    msg.attach_alternative(
        build_template_html(template_filename, context),
        "text/html")
...
def build_template_text(template_filename, context):
    ...
    plaintext = get_template(path.join(
        email_template_dir, template_filename_text))
