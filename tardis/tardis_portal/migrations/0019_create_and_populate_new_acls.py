    ...
    for acl in OBJECTACL.objects.all().iterator():
        ...
        if acl.content_type.model == "experiment":
            try:
                ...
                print(
                    "ERROR finding experiment for new_ACL copy: old_acl_id="
                    + str(acl.id)
                    + ", exp_id="
                    + str(acl.object_id)
                )
        # something went horribly wrong with Generic foreign keys if not an experiment
            ...
            print(
                "bad ACL content type: id="
                + str(acl.id)
                + ",!"
                + str(acl.content_type.model)
                + "!"
            )
        ...
        if acl.pluginId == "django_user":
            try:
                ...
                print(
                    "ERROR finding user for new_ACL copy: old_acl_id="
                    + str(acl.id)
                    + ", userid="
                    + str(acl.entityId)
                )
        ...
        if acl.pluginId == "django_group":
            try:
                ...
                print(
                    "ERROR finding group for new_ACL copy: old_acl_id="
                    + str(acl.id)
                    + ", groupid="
                    + str(acl.entityId)
                )
        ...
        if acl.pluginId == "token_group":
            ...
            print(
                "WARNING  old_acl_id="
                + str(acl.id)
                + " belongs to a token, and thus cannot be converted to the new ACL model"
            )
            continue
