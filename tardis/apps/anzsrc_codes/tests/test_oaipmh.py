...
class RifCSTestCase(TestCase):
    ...
    def testExistsInOaipmh(self):
        ...
        header, metadata = xml.xpath('/o:OAI-PMH/o:GetRecord/o:record/o:*',
                                     namespaces=ns)[0:2]
        self.assertEqual(header.xpath('o:identifier/text()',namespaces=ns)[0],
                         'experiment/1')
        # <registryObject group="MyTARDIS Default Group">
        registryObject = metadata.xpath('r:registryObjects/r:registryObject',
                                           namespaces=ns)[0]
        # <collection type="dataset">
        self.assertEqual(registryObject.xpath(
            'r:collection/@type', namespaces=ns)[0], 'dataset')
        collection = registryObject.xpath('r:collection', namespaces=ns)[0]
        self.assertEqual(collection.xpath('r:subject/@type', namespaces=ns),
                         ['anzsrc-for'])
        self.assertEqual(collection.xpath('r:subject/text()', namespaces=ns),
                         [self.params['code']])
