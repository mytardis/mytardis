...
def _create_test_data():
    user = User(username="testuser", first_name="Voltaire", email="voltaire@gmail.com")
    user.save()
    return (_create_experiment(user, False), _create_experiment(user, True), user)
...
def _create_experiment(user, bad):
    ...
    if bad:
        params = {"type": "website", "identifier": "https://www.badexample.com/"}
    else:
        params = {
            "type": "website",
...
class AbstractExperimentProviderTC:
    ...
    def testGetRecordHandlesInvalidIdentifiers(self):
        for id_ in ["experiment-1", "MyTardis/1"]:
            try:
                self._getProvider().getRecord(self._getProviderMetadataPrefix(), id_)
    ...
    def testListIdentifiers(self):
        ...
        for header in headers:
            ...
            e = (
                self._experiment
                if header.identifier() == _get_first_exp_id()
    ...
    def testListIdentifiersDoesNotHandleSets(self):
        def call_with_set():
            self._getProvider().listIdentifiers(
                self._getProviderMetadataPrefix(), set="foo"
            )
...
class DcExperimentProviderTestCase(AbstractExperimentProviderTC, TestCase):
    ...
    def testGetRecord(self):
        ...
        self.assertEqual(metadata.getField("title"), [str(self._experiment.title)])
        self.assertEqual(
            metadata.getField("description"), [str(self._experiment.description)]
        )
    ...
    def testListRecords(self):
        ...
        for header, metadata, _ in results:
            ...
            self.assertEqual(metadata.getField("description"), [str(e.description)])
    ...
    def testGetRecord(self):
        ...
        ps_id = (
            ExperimentParameterSet.objects.filter(
                experiment=self._experiment, schema__namespace=ns
            )
            .first()
            .id
        )
        ...
        self.assertEqual(
            metadata.getField("licence_uri"), License.get_none_option_license().url
        )
        self.assertEqual(
            metadata.getField("licence_name"), License.get_none_option_license().name
        )
        self.assertEqual(
        ...
        for header, metadata, _ in results:
            ...
            if header.identifier().startswith("experiment"):
                ...
                if e == self._experiment:
                    ...
                    self.assertEqual(list(metadata.getField("related_info")), [{}])
            else:
                ...
                self.assertEqual(metadata.getField("email"), str(self._user.email))
                self.assertEqual(
                    metadata.getField("given_name"), str(self._user.first_name)
                )
                self.assertEqual(
                    metadata.getField("family_name"), str(self._user.last_name)
                )
        # There should have been two
        self.assertEqual(result_count, 2)
        # Remove public flag on first experiment
