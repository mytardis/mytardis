...
class EndpointTestCase(TestCase):
    ...
    def testGetRecord(self):
        ...
        response = self._client_get(
            "/apps/oaipmh/?%s" % "&".join(["%s=%s" % (k, v) for k, v in args.items()])
        )
        self.assertEqual(response.status_code, 200)
        # Check the response content is good
        xml = etree.fromstring(response.content)
        ...
        header, metadata = xml.xpath(
            "/o:OAI-PMH/o:GetRecord/o:record/o:*", namespaces=ns
        )[0:2]
        self.assertEqual(
        ...
        self.assertEqual(
            metadata.xpath("r:registryObjects/r:registryObject/@group", namespaces=ns)[
                0
            ],
            "MyTardis Test Group",
        )
        self._check_experiment_regobj(
            user,
            experiment,
            metadata.xpath("r:registryObjects/r:registryObject", namespaces=ns)[0],
        )
    ...
    def _check_experiment_regobj(self, user, experiment, registryObject):
        ...
        self.assertEqual(
            registryObject.xpath("r:originatingSource/text()", namespaces=ns)[0],
            "http://example.com/",
        )
        # <collection type="dataset">
        self.assertEqual(
            registryObject.xpath("r:collection/@type", namespaces=ns)[0], "dataset"
        )
        ...
        self.assertEqual(
            collection.xpath(
                'r:name[@type="primary"]/r:namePart/text()', namespaces=ns
            )[0],
            experiment.title,
        )
        # <description type="brief">Test experiment description.</description>
        self.assertEqual(
            collection.xpath('r:description[@type="brief"]/text()', namespaces=ns)[0],
        ...
        #   </address>
        # </location>
        loc_xpath = 'r:location/r:address/r:electronic[@type="url"]' + "/r:value/text()"
        self.assertEqual(
    ...
    def _check_user_regobj(self, user, registryObject):
        ...
        self.assertEqual(
            collection.xpath(
                'r:name[@type="primary"]/' + 'r:namePart[@type="given"]/text()',
                namespaces=ns,
            )[0],
            user.first_name,
        )
        self.assertEqual(
            collection.xpath(
                'r:name[@type="primary"]/' + 'r:namePart[@type="family"]/text()',
                namespaces=ns,
            )[0],
            user.last_name,
        )
        ...
        # </location>
        loc_xpath = (
            'r:location/r:address/r:electronic[@type="email"]' + "/r:value/text()"
        )
        self.assertEqual(collection.xpath(loc_xpath, namespaces=ns)[0], user.email)
        # <relatedObject>
        #     <key>user/1</key>
        ...
        self.assertEqual(
            ...
            ["keydomain.test.example/experiment/%d" % Experiment.objects.first().id],
        )

    def testListIdentifiers(self):
        user, experiment = _create_test_data()
        args = {"verb": "ListIdentifiers", "metadataPrefix": "rif"}
        response = self._client_get(
            "/apps/oaipmh/?%s" % "&".join("%s=%s" % (k, v) for k, v in args.items())
        )
        self.assertEqual(response.status_code, 200)
        ...
        assert not xml.xpath("o:error", namespaces=ns)
        idents = xml.xpath(
            "/o:OAI-PMH/o:ListIdentifiers/o:header/o:identifier", namespaces=ns
        )
        assert len(idents) == 2
    ...
    def testListMetadataFormats(self):
        ...
        assert not xml.xpath("o:error", namespaces=ns)
        formats = xml.xpath(
            "/o:OAI-PMH/o:ListMetadataFormats" + "/o:metadataFormat/o:metadataPrefix",
            namespaces=ns,
        )
        self.assertEqual(sorted([fmt.text for fmt in formats]), ["oai_dc", "rif"])
        # With Identifier
        args = {
    ...
    def testListRecords(self):
        ns = self.ns
        user, experiment = _create_test_data()
        args = {"verb": "ListRecords", "metadataPrefix": "rif"}
        response = self._client_get(
            "/apps/oaipmh/?%s" % "&".join("%s=%s" % (k, v) for k, v in args.items())
        )
        self.assertEqual(response.status_code, 200)
        ...
        assert not xml.xpath("o:error", namespaces=ns)
        idents = xml.xpath(
            "/o:OAI-PMH/o:ListRecords" + "/o:record/o:header/o:identifier",
            namespaces=ns,
        )
        assert len(idents) == 2
        assert "experiment/%d" % experiment.id in [i.text for i in idents]
        ...
        collectionObject = xml.xpath(
            metadata_xpath + "/r:registryObjects/r:registryObject" + "[r:collection]",
            namespaces=ns,
        )
        assert len(collectionObject) == 1
        self._check_experiment_regobj(user, experiment, collectionObject[0])
        partyObject = xml.xpath(
            metadata_xpath + "/r:registryObjects/r:registryObject" + "[r:party]",
            namespaces=ns,
        )
