...
class ModelsTestCase(TestCase):
    def setUp(self):
        self.user1 = User.objects.create_user(
            "aperson", email="abc@example.com", password="abc"
        )
        self.user2 = User.objects.create_user(
            "anotherperson", email="def@example.com", password="abc"
        )
        self.user3 = User.objects.create_user(
            "yetanotherperson", email="ghi@example.com", password="abc"
        )
    ...
    def test_credential_generation(self):
        remote_host = RemoteHost.objects.create(
            nickname="dummy host", host_name="localhost", administrator=self.user1
        )
        credential = Credential.generate_keypair_credential(
            self.user1, "remote_user", remote_hosts=[remote_host]
        )
        key = credential.key
        self.assertTrue(
            isinstance(key, PKey),
    ...
    def test_pkey_to_credential(self):
        ...
        credential = Credential.objects.create(
            user=self.user1, remote_user="remote_user"
        )
        credential.key = key
        credential.save()
