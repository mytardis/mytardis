...
class ReplicaAppResource(tardis.tardis_portal.api.ReplicaResource):
    ...
    def prepend_urls(self):
        return [
            re_path(
                ...
                self.wrap_view("recall_dfo"),
                name="hsm_api_recall_dfo",
            ),
            re_path(
                r"^(?P<resource_name>%s)/(?P<pk>\w[\w/-]*)/online%s$"
                % (self._meta.resource_name, trailing_slash()),
                self.wrap_view("dfo_is_online"),
                name="hsm_api_dfo_online",
            ),
        ]
    ...
    def dfo_is_online(self, request, **kwargs):
        ...
        try:
            ...
            return JsonResponse(
                {
                    "error_message": "Recall failed for DFO %s: %s"
                    % (dfo.id, type(err).__name__)
                },
    ...
    def recall_dfo(self, request, **kwargs):
        ...
        try:
            dfo_recall.apply_async(
                args=[dfo.id, request.user.id], priority=dfo.priority
            )
        except HsmException as err:
        ...
        if getattr(settings, "ENABLE_EVENTLOG", False):
            ...
            log(
                action="RECALL",
                extra={"type": "datafile", "id": dfo.datafile.id},
                request=request,
            )

        return JsonResponse({"message": "Recall requested for DFO %s" % dfo.id})
...
class DatasetAppResource(tardis.tardis_portal.api.DatasetResource):
    ...
    def online_count(self, request, **kwargs):
        ...
        try:
            ...
            return JsonResponse(
                {"online_files": online_files, "total_files": total_files}
            )
        except HsmException as err:
            logger.error("Status failed for DS %s: %s" % (dataset.id, str(err)))
            return JsonResponse(
                {
                    "error_message": "Status failed for DS %s: %s"
                    % (dataset.id, type(err).__name__)
                },
                status=HttpResponseServerError.status_code,
            )
    ...
    def dataset_recall(self, request, **kwargs):
        ...
        try:
            ...
            email = EmailMessage(
                subject,
                content,
                to=[settings.RDSM_SUPPORT_EMAIL],
            ...
            return JsonResponse(
                {
                    "error_message": "Recall failed for DS %s: %s"
                    % (ds.id, type(err).__name__)
                },
                status=HttpResponseServerError.status_code,
            )
        ...
        if getattr(settings, "ENABLE_EVENTLOG", False):
            ...
            log(
                action="RECALL", extra={"type": "dataset", "id": ds.id}, request=request
            )

        return JsonResponse({"message": "Recall requested for Dataset %s" % ds.id})
