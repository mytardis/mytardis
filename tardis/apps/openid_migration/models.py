...
class OpenidUserMigration(models.Model):
    old_user = models.ForeignKey(
        User, null=True, related_name="old_user", on_delete=models.CASCADE
    )
    old_user_auth_method = models.CharField(null=False, blank=False, max_length=30)
    new_user = models.ForeignKey(
        User, null=True, related_name="new_user", on_delete=models.CASCADE
    )
...
class OpenidACLMigration(models.Model):
    ...
    acl_id = models.ForeignKey(
        ExperimentACL, on_delete=models.CASCADE, null=True, blank=True
    )
