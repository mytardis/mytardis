...
class TabTestCase(TestCase):
    ...
    def testAccessWithoutReadPerms(self, mock_webpack_get_bundle):
        client = Client()
        response = client.get(
            reverse("tardis.apps.related_info.views.index", args=[self.experiment.id])
        )
...
class ListTestCase(TransactionTestCase):
    ...
    def testHandlesEmptySet(self):
        response = self.client.get(
            reverse(
                "tardis.apps.related_info.views." + "list_or_create_related_info",
    ...
    def testHandlesSingleEntry(self):
        ...
        psm = ParameterSetManager(parentObject=self.experiment, schema=SCHEMA_URI)
        params = {
            "type": "website",
...
class CreateTestCase(TransactionTestCase):
    ...
    def testCanCreate(self):
        ...
        obj = json.loads(response.content.decode())
        self.assertIsInstance(obj["id"], int, "Created object should have an ID.")
        for k, v in params.items():
            self.assertEqual(obj[k], v)
...
class UpdateTestCase(TransactionTestCase):
    ...
    def testMustHaveWrite(self, mock_webpack_get_bundle):
        ...
        self.acl.canWrite = False
        self.acl.save()
        params = {"type": "website", "identifier": "https://www.google.com/"}
        ...
        def do_put(params):
            return self.client.put(
                reverse(
                    "tardis.apps.related_info.views."
                    + "get_or_update_or_delete_related_info",
                    args=[self.experiment.id, self._create_initial_entry()["id"]],
                ),
                data=json.dumps(params),
                content_type="application/json",
            )
