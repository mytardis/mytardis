...
def add_authentication_method(**kwargs):
    ...
    if getattr(settings, "ENABLE_EVENTLOG", False):
        ...
        log(action="USER_LOGIN_SUCCESS", user=user, extra={"auth_method": authMethod})
...
def send_admin_email(**kwargs):
    ...
    if user:
        ...
        try:
            mail.mail_managers(
                subject, message, connection=get_connection(fail_silently=True)
            )
...
def send_account_approved_email(user_id, authMethod):
    ...
    try:
        ...
        user.email_user(subject, message, from_email=from_email, fail_silently=True)
