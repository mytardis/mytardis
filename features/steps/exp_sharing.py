...
def given_a_logged_in_experiment_sharing_user(context):
    ...
    experiment = Experiment.objects.create(title="Shared Experiment1", created_by=user1)
    acl = ExperimentACL.objects.create(experiment=experiment, group=group, canRead=True)
...
def given_a_public_experiment(context):
    ...
    experiment = Experiment.objects.create(title="Public Experiment1", created_by=user1)
